...
- コード内のコメント、docstring、コミットメッセージは（特段の指示がない限り）**日本語** を使用すること
- **識別子は必ず英語（ASCII）表記**にすること。日本語の識別子は使用禁止
  - 対象: `def` で定義する関数名・メソッド名、変数名、引数名、クラス名（他言語の識別子も同様）
  - 日本語で意味を補足したい場合は、名前ではなく **docstring または行コメント** に書く
  - ✅ `def build_step_questions(...):` + `"""解答手順を設問形式へ変換する。"""`
  - ❌ `def 手順設問化(手順): ...`
